...
func (model *Account) RegisterAccountService() error {
	err := db.QueryRow("INSERT INTO accounts (first_name, last_name, email, password) VALUES ($1, $2, $3, $4) RETURNING id", model.FirstName, model.LastName, model.Email, model.Password).Scan(&model.Id)
	if err != nil {
...
func (model *Account) UpdateAccountService() error {
	// accountId trick to get err 404
	var accountId *int
	err := db.QueryRow("UPDATE accounts SET first_name = $1, last_name = $2, email = $3 WHERE id = $4 RETURNING id", model.FirstName, model.LastName, model.Email, model.Id).Scan(&accountId)
...
func (model *Account) DeleteAccountService() error {
	// accountId trick to get err 404
	var accountId *int
	err := db.QueryRow("DELETE FROM accounts WHERE id=$1 RETURNING id", model.Id).Scan(&accountId)
	if err != nil {
		return errors.New("account not found")
	}
